...
with open(f'{argv[2]}-patches.json', "r") as patches_file:
    json = load(patches_file)

for app in json:
        if app['appName'] == argv[1]:
            supportedvers = list(app['versions'])
            break


try:
    for a in fetchurl(f'https://www.apkmirror.com/uploads/?appcategory={argv[1]}').find(['div'], class_="listWidget").find_all(['a'], class_="fontBlack"):
        if argv[3] == "root":
            break
        appver = a.string
