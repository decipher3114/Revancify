        ...
        if [ "$currentPatch" == "none" ]; then
            ...
            while true; do
                ...
                if [ "$infoStatus" == 0 ]; then 
                    ...
                    optionsJson=$(jq -n -r --arg currentPatch "$currentPatch" --argjson optionsJson "$optionsJson" --argjson patchesJson "$patchesJson" '$optionsJson | map((select(.patchName == $currentPatch) | .options) |= [(to_entries[] | .key as $key | .value.value = (if $ARGS.positional[$key] == "" then (first($patchesJson[] | select(.name == $currentPatch)) | .options[$key] | .default) elif $ARGS.positional[$key] == "null" then null elif $ARGS.positional[$key] == "true" then true elif $ARGS.positional[$key] == "false" then false else $ARGS.positional[$key] end)) | .value])' --args "${newValues[@]}")
