...
configure() {
    ...
    CONFIG_OPTS=("LIGHT_THEME" "$LIGHT_THEME" "PREFER_SPLIT_APK" "$PREFER_SPLIT_APK" "LAUNCH_APP_AFTER_MOUNT" "$LAUNCH_APP_AFTER_MOUNT" ALLOW_APP_VERSION_DOWNGRADE "$ALLOW_APP_VERSION_DOWNGRADE" FETCH_PRE_RELEASE "$FETCH_PRE_RELEASE")

    readarray -t UPDATED_CONFIG < <("${DIALOG[@]}" \
        --title '| Configure |' \
        --no-items \
        --separate-output \
        --no-cancel \
